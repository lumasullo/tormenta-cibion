...
class APTMotor():
    def __init__(self, SerialNum=None, HWTYPE=31, verbose=False):
        ...
        HWTYPE_SCC001		21	// 1 Ch stepper driver card (used within
                                   // BSC102,103 units)
        HWTYPE_DCC001		22	// 1 Ch DC servo driver card (used within
                                   // BDC102,103 units)
        ...
        self.unplugged = 'Please connect first! Use initializeHardwareDevice'
...
#        print('APT initialized')
        ...
        self.blCorr = 0.1   # 100um backlash correction
        if SerialNum is not None:
            if self.verbose:
                print("Serial is", SerialNum)
            self.SerialNum = c_long(SerialNum)
            self.initializeHardwareDevice()
        # TODO: Error reporting whether initialisation went sucessfully.
    ...
    def getNumberOfHardwareUnits(self):
        '''
        Returns the number of HW units connected that are available to be
        interfaced
        '''
        numUnits = c_long()
    ...
    def getSerialNumberByIdx(self, index):
        ...
        self.aptdll.GetHWSerialNumEx(self.HWType, hardwareIndex,
                                     pointer(HWSerialNum))
        return HWSerialNum
    ...
    def getHardwareInformation(self):
        ...
        self.aptdll.GetHWInfo(self.SerialNum, model, 255, softwareVersion, 255,
                              hardwareNotes, 255)
    ...
    def getStageAxisInformation(self):
        ...
        stageAxisInformation = [minimumPosition.value, maximumPosition.value,
                                units.value, pitch.value]
    ...
    def setStageAxisInformation(self, minimumPosition, maximumPosition):
        ...
        # Get different pitches of lead screw for moving stages for different
        # stages.
        pitch = c_float(self.config.get_pitch())
        self.aptdll.MOT_SetStageAxisInfo(self.SerialNum, minimumPosition,
                                         maximumPosition, units, pitch)
        return True

    def getHardwareLimitSwitches(self):
        reverseLimitSwitch = c_long()
        forwardLimitSwitch = c_long()
        self.aptdll.MOT_GetHWLimSwitches(self.SerialNum,
                                         pointer(reverseLimitSwitch),
                                         pointer(forwardLimitSwitch))
        hardwareLimitSwitches = [reverseLimitSwitch.value,
    ...
    def setVelocityParameters(self, minVel, acc, maxVel):
        ...
        self.aptdll.MOT_SetVelParams(self.SerialNum, minimumVelocity,
                                     acceleration, maximumVelocity)
        return True
    ...
    def getVelocityParameterLimits(self):
        ...
        maximumVelocity = c_float()
        self.aptdll.MOT_GetVelParamLimits(self.SerialNum,
                                          pointer(maximumAcceleration),
                                          pointer(maximumVelocity))
    ...
    def getPos(self):
        ...
        if not self.Connected:
            raise Exception(self.unplugged)
    ...
    def mcAbs(self, absPosition, moveVel=0.5):
        '''
        Moves the motor to the Absolute position specified at a controlled
        velocity
        absPosition    float     Position desired
        moveVel        float     Motor velocity, mm/sec
        '''
