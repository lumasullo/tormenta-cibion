...
class UpdatePowers(QtCore.QObject):
    ...
    def update(self):
        ...
        cyanPower = np.round(self.widget.cyanlaser.power.magnitude, 1)
        self.widget.cyanControl.powerIndicator.setText(str(cyanPower))
        cyanPSUTemp = np.round(self.widget.cyanlaser.psuTemp.magnitude, 1)
        self.widget.cyanControl.psuTempInd.setText(str(cyanPSUTemp))
        cyanLaserT = np.round(self.widget.cyanlaser.laserTemp.magnitude, 1)
        self.widget.cyanControl.laserTempInd.setText(str(cyanLaserT))
...
class LaserWidget(QtGui.QFrame):
    ...
    def __init__(self, main, lasers, daq, aptMotor, *args, **kwargs):
        ...
        self.main = main
        self.bluelaser, self.greenlaser, self.redlaser, self.cyanlaser = lasers
        self.mW = Q_(1, 'mW')
        self.V = Q_(1, 'V')
        self.daq = daq
        ...
        # Cyan laser control widget
        self.cyanControl = LaserControl(self.cyanlaser, '<h3>473nm</h3>',
                                        color=(0, 183, 255), prange=(0, 1500),
                                        tickInterval=100, singleStep=1,
                                        daq=self.daq, port=1)

        # Additional cyan laser temperature indicators
        self.cyanControl.tempFrame = QtGui.QFrame(self.cyanControl)
        self.cyanControl.tempFrame.setFrameStyle(self.Panel | self.Plain)
        cyanTempGrid = QtGui.QGridLayout()
        self.cyanControl.tempFrame.setLayout(cyanTempGrid)
        self.cyanControl.psuTempInd = QtGui.QLabel('0.0')
        self.cyanControl.psuTempInd.setAlignment(QtCore.Qt.AlignRight)
        self.cyanControl.laserTempInd = QtGui.QLabel('0.0')
        self.cyanControl.laserTempInd.setAlignment(QtCore.Qt.AlignRight)
        cyanTempGrid.addWidget(QtGui.QLabel('Temperature PSU'), 0, 0)
        cyanTempGrid.addWidget(self.cyanControl.psuTempInd, 0, 1)
        cyanTempGrid.addWidget(QtGui.QLabel('ºC'), 0, 2)
        cyanTempGrid.addWidget(QtGui.QLabel('Temperature Laser'), 1, 0)
        cyanTempGrid.addWidget(self.cyanControl.laserTempInd, 1, 1)
        cyanTempGrid.addWidget(QtGui.QLabel('ºC'), 1, 2)
        self.cyanControl.grid.addWidget(self.cyanControl.tempFrame,
                                         2, 0, 1, 2)
        ...
        grid.addWidget(self.cyanControl, 0, 3)
